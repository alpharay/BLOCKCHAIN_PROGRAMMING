...
import "@chainlink/contracts@1.0.0/src/v0.8/shared/interfaces/AggregatorV3Interface.sol"; //importing and using datafeeds from chainlink
...
contract FundMe{//PNB: So here now we can fund a certain account, i.e. 'addressToAmountFunded' with our GWEI value
    ...
    function getVersion() public view returns (uint256){//this function calls the 'version()' function defined in the 'AggregatorV3Interface' contract
        AggregatorV3Interface priceFeed = AggregatorV3Interface(0x694AA1769357215DE4FAC081bf1f309aDC325306);/*the address was copied from the link
