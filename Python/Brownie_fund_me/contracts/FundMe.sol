...
contract FundMe {
    //PNB: So here now we can fund a certain account, i.e. 'addressToAmountFunded' with our GWEI value
    ...
    function fund() public payable {
        //to send ETH funds to an address/account
        //setting a minimum threshold of dollars (e.g. $2) that is sendable
        uint256 minimumUSD = 50 * 10 ** 18; //Converting from USD to WEI; '10**X' is the same as '10EX'
        require(
            getConversionRate(msg.value) >= minimumUSD,
            "You need to spend more ETH!"
        ); // you can accomplish this with an 'if-else' statement but the 'require' function makes this cleaner
    ...
    function withdraw() public payable {
        // to withdraw ETH funds from an address/account
    ...
    function getVersion() public view returns (uint256) {
        //this function calls the 'version()' function defined in the 'AggregatorV3Interface' contract
        return priceFeed.version();
    }

    function getPrice() public view returns (uint256) {
        //this function calls the 'latestRoundData()' function defined in the 'AggregatorV3Interface' contract
    ...
    function getConversionRate(
        ...
        return
            ethAmountInUsd; /* at the time of testing, 32650858802 returned but this is actually 0.000000032650858802 USD 
    by dividing on a calculator by 10E8 to confirm as solidity doesn't give us decimals*/
    }

    function getEntranceFee() public view returns (uint256) {
        // added later o make our lives a little easier when calling it from our python scripts (e.g. in fund_and_withdraw.py)
        // minimum USD
